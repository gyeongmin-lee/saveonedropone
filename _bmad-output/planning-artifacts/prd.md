...
| FR12a | 생성자는 로컬 이미지 파일을 최대 64개까지 배치 업로드하여 항목 이미지를 일괄 추가할 수 있다 | MVP |
| FR13 | 생성자는 항목 이름, 이미지, 선택적 부가 정보(그룹명 등)를 입력할 수 있다 | MVP |
| FR13a | 생성자는 브라켓 시딩 방식을 선택할 수 있다: Randomized(기본) 또는 Preset. Preset 선택 시 큐에서 항목 순서를 드래그로 조정해 시드 순위를 설정한다 | MVP |
